...
class RecipeBook:
    def __init__(self, master):
        ...
        #Отображение страниц
        self.label = Label(text = self.pages, bg = "black", fg = "white")
        self.label.place(relx = 0.95, rely = 0.05)

        #Отображение всех доступных рецептов
        self.combobox = ttk.Combobox(values = self.recipe_texts, state="readonly")
        self.combobox.place(relx = 0.24, rely = 0.05, anchor = "n")
        self.combobox.bind("<<ComboboxSelected>>", self.selected)

    def selected(self, event):
        selection = self.combobox.get()
        print("Выбранный рецепт -", selection)
        self.editor.delete(1.0, END)
        self.editor.insert(1.0, self.combobox.get())
